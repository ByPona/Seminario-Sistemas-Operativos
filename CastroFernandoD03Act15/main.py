...
while True:
    ...
    if len(lista_listos) > 0:
        ...
        while contador_tiempo != obj_tiempo:
            ...
            print("\n\n\n\n\n=========PROCESOS EN NUEVO=============", end="")
            if len(lista_suspendidos)!=0:
                print("\tProcesos en estado suspendido: ",len(lista_suspendidos))
            else:
                print("\n")
            ...
            print("Procesos en Nuevo: ", len(lista_nuevo),end="")
            if len(lista_suspendidos)!=0:
                print("\tPor regresar a memoria: ID: ",lista_suspendidos[0].darid(),"Tamaño: " ,lista_suspendidos[0].dartamano())
            else:
                print("\n")
            ...
            if keyboard.is_pressed('s'):
                ...
                print("==============")
                if len(lista_bloqueado)>0:
                    aux_sus = lista_bloqueado.pop(0)
                    id_sus = aux_sus.darid()
                    cadena = aux_sus.darfinal()
                    lista_suspendidos.append(aux_sus)
                    f = open("suspendidos.txt","w")
                    f.write(cadena)
                    f.close()
                    print("Bloqueado con exito")
                    for x in range(0,48):
                    #print(type(memoria[x].darmarcoid()), type(obj_id))
                        if memoria[x].darmarcoid() == id_sus:
                        #print("encontrado")
                            memoria[x] = marco_libre
                        #os.system("pause")
                    os.system("pause")
                else:
                    print("No hay bloqueados")
                    os.system("pause")
            ...
            if keyboard.is_pressed('r'):
                ...
                print("==============")
                if len(lista_suspendidos)>0:
                    aux_reg = lista_suspendidos.pop(0)
                    print(aux_reg)
                    print("No hay lugar en memoria principal")
                    lista_nuevo.insert(0, aux_reg)
                    os.system("pause")
                else:
                    print("No hay suspendidos")
                    os.system("pause")
